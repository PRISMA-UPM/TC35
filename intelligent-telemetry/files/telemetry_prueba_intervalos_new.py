...
#FIRST_TIMESTAMP_PCAP = 1728552801.0  # 1_min_traffic.pcap
FIRST_TIMESTAMP_PCAP = 1728556659.405  # 5_min_traffic.pcap
SECONDS_DELAY_GRAFANA = 0.0
LAST_INTERVAL = 150 #70 #150
INT_DEBUG=9999 #60
...
t_fich = datetime.now().strftime("%Y%m%d%H%M%S")
NOMBRE_ARCHIVO_POR_SNAPSHOT = f"output_csv/retrasos_snapshot_{t_fich}.csv"
NOMBRE_ARCHIVO_POR_INTERVALO = f"output_csv/retrasos_intervalo_{t_fich}.csv"
...
MAX_rafaga_snapshots_futuro = 750 #250 #1000 # 500 #10000 #500 # 2000 #500 #250 # 500 # 100
...
class AIMonitoring:
    ...
    def write_attack_connection(self, attack_connections_file, src_ip, src_port, dst_ip, dst_port, protocol, timestamp):
        writer = csv.writer(attack_connections_file)
        writer.writerow([
            src_ip,
            src_port,
            dst_ip,
    ...
    def setup_csv_file(self, filename):
        ...
        attack_connections_file = open(filename, mode='a', newline='')

        if not file_exists:
            writer = csv.writer(attack_connections_file)
            writer.writerow(["src_ip", "src_port", "dst_ip", "dst_port", "protocol", "timestamp"])
        
        return attack_connections_file
    ...
    def start_monitoring(self):
        ...
        # WRITING ATTACK CONNECTIONS TO CSV
        #attack_connections_filename = "/usr/app/src/output_csv/attack_connections.csv"
        #attack_connections_file = self.setup_csv_file(attack_connections_filename)
        #---------------------------------
        ...
        if self.crear_csv:
            file_snap = open('snapshots.csv', 'w')
                            ...
                            if adjusted_time >= time_interval_end:
                                ...
                                if adjusted_time < (time_interval_start - timedelta(seconds=5)):
                                    ...
                                    if label == self.label_correspondence["LABEL_HH_MALIGN"]:
                                            ...
                                            security_status = float(num_malign_interval / n_snapshots_dentro_int)
                                            
                                            # WRITING ATTACK CONNECTIONS TO CSV
                                            #self.write_attack_connection(attack_connections_file, metadata["src_ip"], metadata["src_port"], metadata["dst_ip"], metadata["dst_port"], metadata['protocol'], metadata['timestamp'])
