...
def available_cpus():
    """Return CPU IDs available to this process, respecting Linux cpusets."""
...
def set_affinity(idx, cpu_ids=None):
    """ CPU affinity setter """
    if platform.system() == "Linux":
        allowed = available_cpus() if cpu_ids is None else list(cpu_ids)
        temp = list(chunks(allowed, 2))
        x = len(temp)
