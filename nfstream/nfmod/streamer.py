...
from .utils import available_cpus, csv_converter, open_file, RepeatedTimer, update_performances, set_affinity
...
class NFStreamer(object):
    ...
    def __iter__(self):
        allowed_cpus = available_cpus()
        # Pin the streamer without narrowing the workers' inherited CPU set.
        ...
        try:
            ...
            while True:
                ...
                try:
                    try:
                        recv = channel.get(timeout=1)
                    except Empty:
                        failed_meters = [
                            (index, process.exitcode)
                            for index, process in enumerate(meters)
                            if process.exitcode not in (None, 0)
                        ]
                        if failed_meters:
                            for process in meters:
                                if process.is_alive():
                                    process.terminate()
                            child_error = (
                                "NFStream meter process exited unexpectedly: "
                                f"{failed_meters}"
                            )
                            break
                        continue
                    if recv is None:  # termination and stats
                        ...
                        # ----------------------------------------------------------------------------------------------
                            ...
                            if recv.id == self.max_nflows:
                                for process in meters:
                                    process.terminate()
                                break  # Requested max_nflows reached successfully.
                    ...
                    break
            for i in range(n_meters):
                meters[i].join()  # Join and reap every metering job.
            if self._mode == NFMode.INTERFACE and self.performance_report > 0:
                rt.stop()
            if self._mode == NFMode.INTERFACE and self.system_visibility_mode > 0:
